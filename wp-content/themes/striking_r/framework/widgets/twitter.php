...
class Theme_Widget_Twitter extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		foreach($user_array as $key => $user){
			$name= trim($user,'\"');
			$id = $name.'_'.rand(1,1000);
			$just_now=__('just now','striking-r');
			$seconds=__('about %d seconds ago','striking-r');
			$minute=__('about a minute ago','striking-r');
			$minutes=__('about %d minutes ago','striking-r');
			$hour=__('about an hour ago','striking-r');
			$hours=__('about %d hours ago','striking-r');
			$day=__('about a day ago','striking-r');
			$days=__('about %d days ago','striking-r');
			$view_on_twitter=__('view tweet on twitter','striking-r');
			$twitter_script.=<<<SCRIPT
...
jQuery(document).ready(function($) {
	 jQuery("#twitter_wrap_{$id}").tweet({
		...
		just_now_text: "{$just_now}",
		seconds_ago_text: "{$seconds}",
		a_minutes_ago_text: "{$minute}",
		minutes_ago_text: "{$minutes}",
		a_hours_ago_text: "{$hour}",
		hours_ago_text: "{$hours}",
		a_day_ago_text: "{$day}",
		days_ago_text: "{$days}",
		view_text: "{$view_on_twitter}"
	 });
});
...
SCRIPT;
		...
		<?php
			theme_add_js_to_footer($twitter_script);
			echo $after_widget;
		}
	}
