...
class Theme_Widget_Contact_Form extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$id = 'cf_widget_'.rand(100,3000);
		$gdpr_consent = wpml_t(THEME_NAME, 'GDPR Consent',stripslashes(theme_get_option('general','gdpr_consent')));
		if (!empty($gdpr_consent)){
			$gdpr_consent= '<p><input id="contact_'.$id.'_consent" name="contact_consent" tabindex="24" type="checkbox" value="yes"/>' .'<label for="contact_'.$id.'_consent">' .$gdpr_consent. '</label></p>';
		}
		echo $before_widget;
		if ( $title)
		...
		<form class="contact_form" action="<?php echo $action;?>" method="post" novalidate="novalidate">
			...
			<?php echo $gdpr_consent;?>
			<p><button type="submit"<?php echo $bgColor;?> class="<?php echo apply_filters( 'theme_css_class', 'button' );?> white" tabindex="<?php echo empty($gdpr_consent)?"24":"25";?>"><span<?php echo $textColor;?>><?php _e('Submit', 'striking-r'); ?></span></button></p>
