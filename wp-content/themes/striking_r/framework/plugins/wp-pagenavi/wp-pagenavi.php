		...
		case 1:
			...
			if ( !empty( $options['pages_text'] ) ) {
				$pages_text = str_replace(
					...
					__( $options['pages_text'], 'striking-r' ) );
				$out .= "<span class='{$class_names['pages']}'>$pages_text</span>";
			}

			if ( $start_page >= 2 && $pages_to_show < $total_pages ) {
				// First
				$first_text = str_replace( '%TOTAL_PAGES%', number_format_i18n( $total_pages ), __( $options['first_text'], 'striking-r' ) );
			...
			foreach ( $larger_pages_array as $larger_page ) {
				if ( $larger_page < ($start_page - $half_page_start) && $larger_page_start < $larger_page_to_show ) {
					$out .= $instance->get_single( $larger_page, $options['page_text'], array(
						...
						'title' => sprintf( __( 'Page %d', 'striking-r' ), number_format_i18n( $larger_page ) ),
					) );
					$larger_page_start++;
				}
			}
			...
			foreach ( range( $start_page, $end_page ) as $i ) {
				if ( $i == $paged && !empty( $options['current_text'] ) ) {
					...
					$out .= $instance->get_single( $i, $options['page_text'], array(
						...
						'title' => sprintf( __( 'Page %d', 'striking-r' ), number_format_i18n( $i ) ),
					) );
				}
			}
