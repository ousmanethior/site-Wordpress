...
class Theme {
	...
	function after_switch_theme($stylesheet){
		...
		$static_blog_page=theme_get_option_from_db('blog', 'blog_page');
		if (empty($static_blog_page) || $static_blog_page===null ) $static_blog_page='';

		$static_home_page=theme_get_option_from_db('homepage', 'home_page');
		if (empty($static_home_page) ||$static_home_page===null ) $static_home_page='';

		$wp_static_blog_page=get_option( 'page_for_posts');
		$wp_static_home_page=get_option( 'page_on_front');

		$reading_settings_changed='false';
		
		if ($wp_static_blog_page!=$static_blog_page) {
			update_option( 'page_for_posts', $static_blog_page );
			$reading_settings_changed='true';
		}
		if ($wp_static_home_page!=$static_home_page) {
			update_option( 'page_on_front', $static_home_page );
			$reading_settings_changed='true';
		}
		if ($reading_settings_changed=='true') {
			if (empty($static_home_page) && empty($static_blog_page)) {
				update_option( 'show_on_front', 'posts' );
			} else 	update_option( 'show_on_front', 'page' );
		}
