...
HTML;
		...
		foreach($images as $image) {
			...
			if(isset($image['link']) && $image['link'] != false){
				$output .= '<a href="'.$image['link'].'" target="'.$image['target'].'" class="nivo-image-link"><img src="'.$image_src.'"'.$srcset.' title="'.$desc.'" alt="" /></a>';
			}else{
				$output .= '<img src="'.$image_src.'"'.$srcset.' title="'.$desc.'" alt="" />';
			}
			$i++;
		}
...
HTML;
	    ...
	    if (!empty($nivo_css)) theme_add_css_to_footer($nivo_css);
		return $output;
	}
}
