...
<p><b>WP SEO NOTE:</b> &nbsp;&nbsp;If you are using WP SEO plugin to customize the blog breadcrumbs trail, then you would need to set a static blog page.</p>",'theme_admin'),
				"options" => array(
					...
					array(
						...
						"desc" => __("There are 5 featured image positions: Full Width, Left Float, Right Float, Between and Below Title. &nbsp;Below Title refers to the post title and the meta appearing first, with the featured image below this information. &nbsp;Please see the demo for an example of this appearence.",'theme_admin'),
						...
						"options" => array(
							...
							"between" => __('Between Title & Meta','theme_admin'),
						),
						"type" => "select",
					),
					array(
					...
					array(
						"name" => __("Strip shortcodes from the excerpt",'theme_admin'),
						"desc" => __("If this option is ON, it will remove the shortcodes from the excerpt in the blog listing pages. This only works when Enable Shortcode for excerpt and Display Full Blog Posts are turned OFF. Note: Only applies to the real excerpt. Not the excerpt generated from the content when no excerpt has been added.",'theme_admin'),
						"id" => "strip_shortcode_excerpt",
						"default" => false,
						"type" => "toggle"
					),
					array(
...
<p>The settings in this resource tab apply to how any single blog post page will appear to the viewer. &nbsp;These are global default settings, and in most instances, the Blog Metabox (found below the WP content editor) contains duplicate settings to provide an override of the defaults below for those instances where a different appearence is required.</p>",'theme_admin'),
				"options" => array(
					...
					array(
						...
						"desc" => __('If set to "Use Static Blog page Settings" The featured header will be the same as the featured header as set in the page that has been set as you default blog listing page in the theme settings.','theme_admin'),
						"id" => "single_header",
						"default" => 'single',
						"options" => array(
					...
					array(
						...
						"desc" =>  __("If the previous option is ON, and this option is ON, the Previous & Next Navigation will only show the portfolio items with the same Category.",'theme_admin'),
			...
			array(
				...
				"options" => array(
					array(
						"name" => __("Meta Information Icons",'theme_admin'),
						"desc" => __("If Meta Information Icons is set to ON the meta information in blog lists and single posts are preceded by icons instead of text.",'theme_admin'),
						"id" => "meta_info_icons",
						"default" => true,
						"type" => "toggle"
					),
			...
			array(
				...
				"options" => array(
					...
					array(
						"name" => __("Link Excerpt Text to Post",'theme_admin'),
						"desc" => __("If set to ON the complete excerpt text will link to the post itself. Note: this will strip all tags from the excerpt and you can not use shortcodes within the excerpt. Only applies to excerpt and the excerpt can only have clean text with paragraphs or break-tags. This setting only works when the Display &#34;Read More&#34; setting above is turned OFF.",'theme_admin'),
						"id" => "link_excerpt",
						"default" => false,
						"type" => "toggle"
					),
