...
<p>There is also a tritoggle setting in the Portfolio metabox which allows for override of this setting on post-by-post basis -> the benefit of the tritoggle is that if this setting is ON, the tritoggle has an OFF setting, and if this setting is OFF, the tritoggle has an ON setting, so no post is ever restricted to the theme default setting. &nbsp; The third position of the tritoggle is of course Default so that the post mimics the default behaviour of this setting.</p>",'theme_admin'),
					...
					array(
						...
						"desc" => __("<p>If this setting is ON, clicking on the Featured Image in a single portfolio post will show ALL to the portfolio item attached images (by media insert uploaded) in a Lightbox Gallery Popup including the featured image. You can restrict the images that are shown in the Lightbox popup to Gallery Only Images when the single portfolio item is a type 'Gallery' in the next setting.</p>",'theme_admin'),
						"id" => "featured_image_lightbox_gallery",
						"default" => false,
						"type" => "toggle"
					),
					array(
						"name" => __("Only Show Gallery Images",'theme_admin'),
						"desc" => __("<p>If this setting is ON, and the portfolio item is a portfolio type Gallery, the Gallery Lightbox will show, besides the featured image, only the images attached in the Gallery of that portfolio item. If this setting is OFF the Lightbox Gallery will show all images attached (by media insert uploaded) to that the single portfolio item as a Gallery.  Note: this only applies to the Gallery in a single portfolio item and not to the Gallery as presented in a Lightbox popup in a Portfolio List. The latter will only show the images attached to the Gallery of the single Portfolio item.</p>",'theme_admin'),
						"id" => "only_gallery_images",
						"default" => false,
						"type" => "toggle"
					),
					array(
