		...
		$options = array(
			...
			array(
				...
				"options" => array(
					...
					array(
						"name" => __("Global Single Portfolio Post Page Custom Sidebar Selector",'theme_admin'),
		...
		if(function_exists('is_post_type_archive')){
			...
			if ($post_types) {
				...
				if(!empty($post_types)){
					...
					foreach ($post_types as $post_type ) {
						if($post_type->name != 'portfolio'){
							if ($post_type->labels->name=='Products' && class_exists('Woocommerce')) {
								$suffix='and Shop Page ';
							} else $suffix='';
							$post_types_option['options'][] = array(
								"id" => 'post_type_archive_'.$post_type->name.'_sidebar',
								"name" => sprintf(__("%s's Archive Page %sSidebar",'theme_admin'),$post_type->labels->name,$suffix),
								"desc" => '',
								"default" => '',
								"options"=> theme_get_sidebar_options(),
								"prompt" => __("None",'theme_admin'),
