...
$custom_script = <<<HTML
	...
	var lightboxgroup = attrs['lightboxGroup'].value;
	var touch = attrs['touch'].value;
	var speed = attrs['speed'].value;
	var delay = attrs['delay'].value;
	var link_target = attrs['link_target'].value;
	var showCaptions = attrs['showCaptions'].value;
	var textColor = attrs['textColor'].value;
	var bgColor = attrs['bgColor'].value;
	var captionSize = attrs['captionSize'].value;
	var captionPos = attrs['captionPos'].value;
	var captionMargin = attrs['captionMargin'].value;
	var effect = attrs['effect'].value;
	var icon = attrs['icon'].value;
	var number_attr = '';
	...
	if(lightboxgroup === 'true'){
		lightboxgroup = ' lightboxGroup="'+lightboxgroup+'"';
	}else{
		lightboxgroup = '';
	}
	if(effect!='icon') {
	...
	if(showCaptions === 'true'){
		showCaptions = ' showCaptions="'+showCaptions+'"';
	}else{
		showCaptions = '';
	}
	if(textColor!='') {
	...
	if(captionSize!='' && captionSize!=12) {
		captionSize = ' captionSize="'+captionSize+'"';
	} else captionSize='';
	if(captionPos!='bottom') {
		captionPos = ' captionPos="'+captionPos+'"';
	} else captionPos='';
	if(captionMargin!='0') {
		captionMargin = ' captionMargin="'+captionMargin+'"';
	} else captionMargin='';
	var ret = '\\n[carousel width="'+attrs['width'].value+'" height="'+attrs['height'].value+'"'+source+number_attr+title+nav+autoplay+circular+touch+delay+speed+lightbox+lightboxgroup+effect+icon+link_target+showCaptions+textColor+bgColor+captionSize+captionPos+captionMargin+']\\n';
	var caption = '';
	var imgSource = '';
