...
$init_script = <<<HTML
	jQuery('[name="controls"]').on("change",function(){
		if(this.checked){
			...
			jQuery('.shortcode-item[data-option="draggable"]').show();
		}else{
			...
			jQuery('.shortcode-item[data-option="draggable"]').hide();
		}
	}).trigger("change");
HTML;
$custom_script = <<<HTML
...
return array(
	...
	"options" => array(
		...
		array (
			"name" => __("Black and White Map",'theme_admin'),
			"id" => "blackWhite",
			"desc" => __("Show the map in Black and White colors. Any Map Type set is ignored and ROADMAP is used. Note: You need to turn OFF MapTypeControl.", "theme_admin"),
			"default" => false,
			"type" => "toggle"
		),
		array (
		...
		array(
			"name" => __("Draggable (Optional)&#x200E;",'theme_admin'),
			"id" => "draggable",
			"default" => 'true',
			"options" => array(
				"true" => __('Dragging Map Enabled','theme_admin'),
				"false" => __('Dragging Map Disabled','theme_admin'),
				"979" => __('Disable Dragging Below 980px','theme_admin'),
				"767" => __('Disable Dragging Below 768px','theme_admin'),
				"567" => __('Disable Dragging Below 568px','theme_admin'),
				"479" => __('Disable Dragging Below 480px','theme_admin'),
			),
			"type" => "select",
		),
		array (
