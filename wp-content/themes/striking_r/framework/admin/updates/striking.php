...
if(class_exists('Woocommerce')) {
	if(!get_option(THEME_SLUG.'_woocommerce_330_image_settings_updated')){
		global $woocommerce;
		global $woo_config;
		...
		if(version_compare($wooversion, "3.3.0", '>=')){
			...
			$woocommerce_settings=theme_get_option('advanced');
			if (!is_array($shop_catalog_image_size)){
				$shop_catalog_image_size=array('width'=>$woo_config['full']['shop_catalog']['width'],'height'=>$woo_config['full']['shop_catalog']['height'],'crop'=>0);
			}
			if (isset($shop_catalog_image_size['width'])) $woocommerce_settings['woocommerce_shop_image_width']=$shop_catalog_image_size['width'];
			if (isset($shop_catalog_image_size['height'])) $woocommerce_settings['woocommerce_shop_image_height']=$shop_catalog_image_size['height'];
			if (isset($shop_catalog_image_size['crop'])) {
				if($shop_catalog_image_size['crop']==1) $woocommerce_settings['woocommerce_shop_image_crop']=true;
				else $woocommerce_settings['woocommerce_shop_image_crop']=false;
			}
			
			if (!is_array($shop_single_image_size)){
				$shop_single_image_size=array('width'=>$woo_config['full']['shop_single']['width'],'height'=>$woo_config['full']['shop_single']['height'],'crop'=>1);
			}
			if (isset($shop_single_image_size['width'])) $woocommerce_settings['woocommerce_single_image_width']=$shop_single_image_size['width'];
			if (isset($shop_single_image_size['height'])) $woocommerce_settings['woocommerce_single_image_height']=$shop_single_image_size['height'];
			if (isset($shop_single_image_size['crop'])) {
				if($shop_single_image_size['crop']==1) $woocommerce_settings['woocommerce_single_image_crop']=true;
				else $woocommerce_settings['woocommerce_single_image_crop']=false;
			}

			if (!is_array($shop_thumbnail_image_size)){
				$shop_thumbnail_image_size=array('width'=>$woo_config['full']['shop_thumbnail']['width'],'height'=>$woo_config['full']['shop_thumbnail']['height'],'crop'=>0);
			}
			if (isset($shop_thumbnail_image_size['width'])) $woocommerce_settings['woocommerce_thumbnail_image_width']=$shop_thumbnail_image_size['width'];
			if (isset($shop_thumbnail_image_size['height'])) $woocommerce_settings['woocommerce_thumbnail_image_height']=$shop_thumbnail_image_size['height'];
			if (isset($shop_thumbnail_image_size['crop'])) {
				if($shop_thumbnail_image_size['crop']==1) $woocommerce_settings['woocommerce_thumbnail_image_crop']=true;
				else $woocommerce_settings['woocommerce_thumbnail_image_crop']=false;
			}
			
			update_option( 'theme_' . 'advanced', $woocommerce_settings);
			
			update_option(THEME_SLUG.'_woocommerce_330_image_settings_updated', true);
		}
	}
}