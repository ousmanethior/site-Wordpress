...
class Theme_Options_Page_With_Tabs extends Theme_Options_Page {
	...
	function _render(){
		...
		if (is_child_theme()) {
			...
			$version = $child_theme_info->get( 'Version' );
			if (!empty($version)) $version='<small>version '.$version.'</small>';
			$child_theme_name = $child_theme_info->get( 'Name' );
			$child_theme_name = '<h3>'.$child_theme_name.'</h3>'.$version;
		} else $child_theme_name='';
