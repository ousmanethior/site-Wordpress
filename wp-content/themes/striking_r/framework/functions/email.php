...
$siteurl =  esc_url( home_url() );//get_bloginfo('url');
$gdpr_consent_text = wpml_t(THEME_NAME, 'GDPR Consent',stripslashes(theme_get_option('general','gdpr_consent')));
...
$gdpr_consent = isset($_POST['consent'])?trim($_POST['consent']):'';
...
if (!empty($gdpr_consent)) {
	if ($gdpr_consent=='true') $gdpr_consent=__('Yes','striking-r'); 
	else if ($gdpr_consent=='false') $gdpr_consent=__('No','striking-r'); 
	else $error = true;
}
...
if($error == false){
	...
	if (!empty($gdpr_consent_text)) {
		$body .= $gdpr_consent_text.': '.$gdpr_consent."\n\n";
	}
	$body .= __('User IP: ','striking-r').$user_ip."\n\n";
	$body .= __('Referrer: ','striking-r').$referrer."\n\n";
