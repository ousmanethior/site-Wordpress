    ...
    items.each(function() {
        ...
        if (thumbnail) {
            ...
            function update() {
                ...
                var woo_variable_image=$image.attr("data_woo_variable_image");
                var size = width + '.' + height;
                if (!images||typeof woo_variable_image !== 'undefined') {
                    images = [];
                }
				if (typeof woo_variable_image !== 'undefined') {
                    var tmp_width=$image.attr("width");
                    var tmp_height=$image.attr("height");
                    if ((width/height) != (tmp_width/tmp_height) ){
                      var rect = $image[0].getBoundingClientRect();
                      var imgContainerWidth;
                      if (rect.width) {
					     // `width` is available for IE9+
                        imgContainerWidth = rect.width;
                      } else {
                         // Calculate width for IE8 and below
                        imgContainerWidth = rect.right - rect.left;
                      }
                      if (typeof imgContainerWidth !== 'undefined') {
                        height=parseInt((imgContainerWidth/tmp_width)*tmp_height,10);
                        width=parseInt(imgContainerWidth,10);
                      } else {
                        height=parseInt((width/tmp_width)*tmp_height,10);
                      }
                    } 
                }
                ...
                if (width === $image[0].naturalWidth && height === $image[0].naturalHeight && typeof woo_variable_image === 'undefined') {
                        ...
                        jQuery.post(window.location.href, {
                            ...
                            tmpImg.onload = function() {
								...
								if (typeof srcset !== typeof undefined && srcset !== false) {
									if(srcset.indexOf("@2x") != -1 && srcset.indexOf(" 2x") != -1) {
										var fileExtension = data.substring(data.lastIndexOf('.'));
										var fileName = data.substr(0, data.lastIndexOf('.'));
										//var newsrcset=data+', '+fileName+'@2x'+fileExtension+' 2x';
										var newsrcset=fileName+'@2x'+fileExtension+' 2x';
...
jQuery(document).ready(function($) {
        ...
        $('.single-product.woocommerce .variations a.reset_variations').click(function(){
            $("img.attachment-shop_single.wp-post-image.on_the_fly_resize").each(function() {
                ...
                if (typeof default_thumb != "undefined") {
                    ...
                    themeUpdateImages($(".woocommerce-main-image img.on_the_fly_resize"));
                }
            });
        });
    });
    ...
    $('.milestone_number').on('scrollSpy:enter', function() {
	    var numberOfAnimations = $(this).data('animations');
		if (numberOfAnimations== undefined) numberOfAnimations=1;
		var countAnimations = $(this).data('countAnimations');
		if (countAnimations == undefined ) {
			$(this).data('countAnimations', 1);
		} else {
			countAnimations++;
			$(this).data('countAnimations', countAnimations);
		}
        if (!$(this).data('visibled')) {
			if (countAnimations>=numberOfAnimations && numberOfAnimations!=0)$(this).data('visibled', 1);
    ...
    $('.progress').on('scrollSpy:enter', function() {
        ...
        if (!$(this).data('visibled')) {
			...
			$(this).find('.progress-meter').css('width','');
    ...
    $(".vertical_tabs_container").each(function() {
		...
		var $len=$("ul.vertical_tabs li, ul.theme_vertical_tabs li",this).length;
		if ($initialIndex==-1) $initialIndex=0;
		if ($initialIndex>($len-1))$initialIndex=($len-1);
    ...
    $('.responsive_text').each(function() {
			...
			if (tabs.length != 0) {
				tabs.each(function() {
					...
					tabs.find('.current').on('change', function() {
    ...
    var enable_lightbox = function(selector, parent) {
        ...
        $(parent).find(selector).fancybx(options);
    };
    ...
    enable_lightbox('.fancybx, .colorbox', document);
    ...
    $(".gallery .gallery-image,.carousel .carousel-image,.woo-image-overlay .image-overlay").imagesLoaded(function(instance) {
