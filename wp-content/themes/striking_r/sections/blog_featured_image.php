...
function theme_section_blog_featured_image($type='full', $width = '', $height='', $frame = false, $effect= '', $single = false){
	...
	if($single){
		if(theme_get_option('blog', 'featured_image_lightbox')){
			...
			if($effect!='none'){
				$output .= '<a class="image_icon_zoom" href="#" title="'.$title.'"><img width="'.$width.'" height="'.$height.'" data-thumbnail="'.$thumbnail_id.'" src="'.$image_src.'"'.$srcset.' alt="'.$title.'" /></a>';
			}else{
